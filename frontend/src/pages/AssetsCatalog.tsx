...
export default function AssetsCatalog() {
  ...
  console.log('🚀 AssetsCatalog INIT - bundle id: ASSETS_BUNDLE_VANILLA_REFACTOR')
  ...
  // Refactor: Logic for external button clicks delegated to table actions
  useEffect(() => {
    const container = hotTableRef.current
    if (!container) return
  ...
  const initializeHandsontable = () => {
    ...
    hotInstance.current = new Handsontable(hotTableRef.current, {
      ...
      columns: [
        { data: 'symbol', readOnly: true, width: 120, className: 'htLeft htBold' },
        { data: 'name', readOnly: true, width: 250 },
        { data: 'asset_type', readOnly: true, width: 100 },
        { data: 'market', readOnly: true, width: 100 },
